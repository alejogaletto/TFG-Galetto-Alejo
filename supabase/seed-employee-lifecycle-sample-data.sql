-- =====================================================
-- Employee Lifecycle Hub - Sample Data Seed Script
-- =====================================================
-- Inserta datos ficticios en español para demos.
-- Reemplaza v_user_id con el ID del usuario que verá los datos.
-- Ejecutar después de crear la base de datos (y opcionalmente formularios).
-- =====================================================
...
BEGIN
  ...
  IF v_user_id = 'REPLACE_WITH_USER_ID' THEN
    RAISE EXCEPTION 'Actualiza v_user_id con el UUID del usuario antes de ejecutar este seed.';
  END IF;
  ...
  SELECT vts.id INTO v_equipamiento_table_id
  FROM "VirtualTableSchema" vts
  JOIN "VirtualSchema" vs ON vts.virtual_schema_id = vs.id
  WHERE vs.template_type = 'employee_lifecycle' AND vs.is_template = true AND vts.name = 'Equipamiento'
  LIMIT 1;

  IF v_empleados_table_id IS NULL THEN
    RAISE EXCEPTION 'No se encontró la tabla Empleados. Ejecuta seed-employee-lifecycle-database.sql primero.';
  END IF;
  ...
  RAISE NOTICE 'Empleados insertados: 6';
  ...
  RAISE NOTICE 'Tareas insertadas: 5';
  ...
  RAISE NOTICE 'Checklists insertados: 3';
  ...
  RAISE NOTICE 'Equipamiento insertado: 4';

  RAISE NOTICE '================================================';
  RAISE NOTICE 'Datos de ejemplo creados para el Employee Lifecycle Hub.';
  RAISE NOTICE '================================================';
